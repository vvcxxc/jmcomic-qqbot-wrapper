...
>
> - **解压密码 = 车号**（如 `JM350234` 的密码就是 `350234`）。机器人上传后会在群里回 `解压密码：350234`。
> - 群友下载 `JM350234.zip`，用车号当密码解压，里面就是原来带漫画名的 zip。
> - 加密用 `pyzipper`（AES），内层已是压缩包故 `ZIP_STORED` 不二次压缩，体积基本不变。外层写在 `downloads\zip`（已挂载进容器），上传用容器路径。
