...
效果：下载 → 打包 zip → **AES 加密压缩** → 上传群文件，并在群里回出解压密码。

> **群文件加密防和谐**：群文件上传后，QQ 服务器会把 zip 解开扫描里面的图，普通压缩包（哪怕套几层、改文件名）都防不住——内容被判违规就"下载失败"。所以群聊上传时把内层 zip（带漫画名的那个）用 **AES 加密**再套一层，外层文件名只用 `JM车号.zip`。QQ 解不开加密包、读不到内容，下载就正常。
>
> - **解压密码 = 车号**（如 `JM1234042` 的密码就是 `1234042`）。机器人上传后会在群里回 `解压密码：1234042`。
> - 群友下载 `JM1234042.zip`，用车号当密码解压，里面就是原来带漫画名的 zip。
> - 加密用 `pyzipper`（AES），内层已是压缩包故 `ZIP_STORED` 不二次压缩，体积基本不变。
> - **私聊不加密**（私聊不和谐），仍直接发带漫画名的 zip，不用输密码。
