...
> **群文件二层打包（防和谐）**：群文件容易因文件名/内容被和谐，所以群聊上传时会把内层 zip（带漫画名的那个）再套一层，**外层只用 `JM车号.zip` 命名、不含漫画名**，上传外层。群友下载 `JM350234.zip` 解开后，里面就是原来带漫画名的 zip。外层不二次压缩（`ZIP_STORED`），体积不变。外层写在 `downloads\zip`（已挂载进容器），上传用容器路径。**私聊不做这层包装**，仍直接发带漫画名的 zip。
